...
⚠️ Ce script N'écrit PAS dans prospection.csv. Il écrit chaque résultat
dans extraction_results.jsonl (une ligne JSON par agence). Pour appliquer
les résultats à prospection.csv : merge_results.py.
...
def main():
    ...
    parser.add_argument("--force", action="store_true",
                         help="Relance même les agences déjà présentes dans extraction_results.jsonl")
    ...
    # Numéro stable sur l'ensemble du CSV, basé sur la position parmi les
    # agences qui ONT un site (présentes dans pages_cache.jsonl) — pas sur
    # l'ordre des lignes traitées CE run. Comme prospection.csv et le cache
    # ne changent pas d'un run à l'autre, cette position est la même à
    # chaque run : c'est ce qui permet d'afficher "[665/6024]" dès la
    # première ligne d'un nouveau run, au lieu de recompter à partir de 1.
    total_with_site = sum(1 for row in rows if row.get(nom_col, "").strip() in cache)
    agency_counter = 0

    print(f"📄 {len(rows)} lignes dans {CSV_PATH} (lecture seule — jamais modifié par ce script)")
    print(f"📦 {len(cache)} agences en cache de pages ({total_with_site} lignes du CSV concernées)")
    ...
    for row in rows:
        ...
        # Compte cette agence dans la position globale AVANT de savoir si
        # elle est déjà faite : c'est ce qui garde le numéro synchronisé
        # d'un run à l'autre (664 à la fin d'un run -> 665 au run suivant),
        # même si les agences déjà faites sont sautées sans être imprimées.
        agency_counter += 1
        ...
        print(f"[{agency_counter}/{total_with_site}] {nom} ({len(pages)} page(s))")
    ...
    print(f"\n✅ Terminé : {processed} lignes traitées cette fois "
          f"(position atteinte : {agency_counter}/{total_with_site}). "
    ...
    print(f"👉 Résultats écrits dans {RESULTS_PATH}. prospection.csv n'a PAS été modifié.")
