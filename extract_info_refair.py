#!/usr/bin/env python3
"""
ÉTAPE 2 : lit les pages nettoyées dans pages_cache.jsonl (produites par
scrape_pages.py) et utilise GPT-OSS-120B (via Cerebras) pour en extraire,
AGENCE PAR AGENCE, les infos suivantes :
  - Adresse
...
⚠️ Variante "refair" : lit refair_avec_site_web.csv / pages_cache_refair.jsonl
et n'écrit PAS dans prospection.csv ni dans extraction_results.jsonl (le
fichier principal). Chaque résultat est écrit dans
extraction_results_refair.jsonl (une ligne JSON par agence), séparément du
run principal pour ne rien mélanger. Pour appliquer ces résultats-là au CSV,
adapte merge_results.py (ou fusionne extraction_results_refair.jsonl dans
extraction_results.jsonl avant de lancer merge_results.py normalement).

MISE À JOUR RATE LIMITING (Cerebras : TPM ET RPM par clé) :
  - Un limiteur de débit par clé API suit, sur une fenêtre glissante de
    60s, à la fois le nombre réel de tokens consommés (via le champ
    "usage" que Cerebras renvoie) et le nombre de requêtes envoyées, et
    attend automatiquement avant d'envoyer une requête qui dépasserait
    le budget (tokens ou requêtes) de la clé utilisée.
  - Si le contenu d'une agence est trop volumineux pour tenir dans une
    seule requête sans à elle seule saturer le budget, il est découpé en
    plusieurs morceaux (par page, et si besoin une page est elle-même
    coupée). Chaque morceau est envoyé séparément à l'IA, qui est
    prévenue qu'il s'agit d'une partie du site d'une même agence. Les
    résultats des morceaux sont ensuite fusionnés (premier champ non vide
    retenu, source de page conservée).
"""
...
CSV_PATH = "refair_avec_site_web.csv"
CACHE_PATH = "pages_cache_refair.jsonl"
RESULTS_PATH = "extraction_results_refair.jsonl"
...
ROW_DELAY_SECONDS = 0.5      # petite pause entre lignes, en plus du limiteur
...
def find_nom_column(fieldnames):
    ...
    if actual is None:
        print(f"❌ Colonne « {NOM_COLUMN_LABEL} » introuvable dans {CSV_PATH}.")
        print(f"   Colonnes trouvées : {fieldnames}")
        sys.exit(1)
    return actual
...
def load_pages_cache():
    ...
    if not os.path.exists(CACHE_PATH):
        sys.exit(f"❌ {CACHE_PATH} introuvable — lance d'abord scrape_pages.py")
...
def save_result(nom, result, pages, url_list):
    record = {
        "nom": nom,
...
def process_agency(nom, pages, api_keys, alive, key_ptr, limiters, daily_quota_flag):
    """Traite une agence (éventuellement découpée en plusieurs chunks),
    en respectant le budget de tokens/minute de la clé utilisée à chaque
    appel. daily_quota_flag est un dict partagé {"hit": bool} mis à jour
    si une clé tombe en quota JOURNALIER épuisé (par opposition à une clé
    vraiment invalide). Renvoie (result_merged|None, new_key_ptr)."""
    chunks = build_chunks(pages, MAX_CHUNK_TOKENS)
    chunk_total = len(chunks)
    chunk_results = []

    if chunk_total > 1:
        print(f"    ✂️ Agence volumineuse : découpée en {chunk_total} morceaux")
    ...
    for i, chunk_items in enumerate(chunks, start=1):
        ...
        chunk_done = False
        attempts = 0
        while not chunk_done and alive and attempts < len(api_keys):
            attempts += 1
            pos = key_ptr % len(alive)
            key_idx = alive[pos]
            key_ptr += 1
            api_key = api_keys[key_idx]
            limiter = limiters[key_idx]
        ...
        if not chunk_done:
            # plus de clé vivante en cours de traitement des chunks
            return None, key_ptr
...
def main():
    ...
    parser.add_argument("--force", action="store_true",
                         help="Relance même les agences déjà présentes dans extraction_results_refair.jsonl")
    ...
    alive = list(range(len(api_keys)))
    limiters = {
        i: KeyRateLimiter(tpm_limit=args.tpm_limit, rpm_limit=args.rpm_limit)
    ...
    nom_col = find_nom_column(fieldnames)
    print(f"📄 {len(rows)} lignes dans {CSV_PATH} (lecture seule — jamais modifié par ce script)")
    print(f"📦 {len(cache)} agences en cache de pages")
    print(f"📝 Résultats déjà enregistrés dans {RESULTS_PATH} : {len(already_done)}")
    print(f"🔑 {len(api_keys)} clés Cerebras chargées "
          f"(limite : {args.tpm_limit} tokens/min et {args.rpm_limit} requêtes/min par clé)\n")

    processed = 0
    skipped_no_cache = 0
    skipped_already_done = 0
    stopped_no_keys = False
    daily_quota_flag = {"hit": False}

    # Position (1-based) de la première ligne pas encore faite, juste pour
    # informer clairement d'où on reprend (l'itération elle-même saute déjà
    # les lignes faites sans aucun coût notable, quel que soit leur nombre).
    premiere_ligne_restante = None
    for idx, row in enumerate(rows, start=1):
        n = row.get(nom_col, "").strip()
        if n and n not in already_done:
            premiere_ligne_restante = idx
            break
    if premiere_ligne_restante:
        print(f"▶️ Reprise à partir de la ligne {premiere_ligne_restante}/{len(rows)} du CSV\n")

    for global_idx, row in enumerate(rows, start=1):
        if processed >= args.limit:
            break
        if time.time() - start_time >= time_budget_seconds:
            print(f"\n⏱️ Budget de temps ({args.time_budget_minutes} min) atteint, arrêt propre.")
            break
        if not alive:
            print("\n🛑 Plus aucune clé Cerebras fonctionnelle, arrêt.")
            stopped_no_keys = True
            break
        ...
        pages = normalize_pages(cache[nom].get("pages", []))
        url_list = [p.get("url", "") for p in pages]

        print(f"[{global_idx}/{len(rows)}] {nom} ({len(pages)} page(s))")
        result, key_ptr = process_agency(nom, pages, api_keys, alive, key_ptr, limiters, daily_quota_flag)

        if result:
            log_extraction(nom, result, pages)
            save_result(nom, result, pages, url_list)
            already_done.add(nom)
        else:
    ...
    remaining = any(
        row.get(nom_col, "").strip() in cache
        and row.get(nom_col, "").strip() not in already_done
        for row in rows
    )
    if stopped_no_keys:
        remaining = True
    ...
    dead_count = len(api_keys) - len(alive)
    print(f"\n✅ Terminé : {processed} lignes traitées cette fois. "
          f"{skipped_already_done} déjà faites, {skipped_no_cache} sautées (pas en cache). "
          f"Clés mortes ce run : {dead_count}/{len(api_keys)}. Reste à faire : {remaining}")
    ...
    print(f"👉 Résultats écrits dans {RESULTS_PATH}. {CSV_PATH} n'a PAS été modifié.")
    print("   Pour appliquer les résultats au CSV : python merge_results.py")
